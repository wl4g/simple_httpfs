...
class CustomBaseHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        dir_path = base_dir   # configured basedir path.
    ...
    def do_get_index_page(self, is_redirect):
        ...
        request_path = base_dir + re.split(r'\?|\#', self.path)[0]

        if is_redirect:
            self.send_response(301)
            schema = self.headers.get('X-Forwarded-Proto', "http://")
            hostAndPort = self.headers.get('Host', "")
            rootPath = schema + hostAndPort
            print("redirect to {} ...".format(rootPath))
            self.send_header("Location", rootPath)
            self.end_headers()  # the response to browser
            return

        # Response files html
        if os.path.isdir(request_path):
            directory_contents_html = self.get_directory_contents_html(
                request_path)
            print(directory_contents_html)

            self.send_response(200)
            self.send_header("Content-type", "text/html")
            self.send_header("Content-Length",
                             str(len(directory_contents_html)))
            self.end_headers()
        ...
        try:
            ...
            self.send_response(200)
            self.send_header("Content-Length",
                             str(os.stat(request_path).st_size))
            self.send_header(
                "Last-Modified", self.date_time_string(os.stat(request_path).st_mtime))
            if request_path.endswith(('.gif', '.jpg', '.png', '.jpeg', '.bmp', '.webp', '.ico')):
                self.send_header("Content-type", "image/jpeg")
            elif request_path.endswith(('.mp4', '.w4a', '.w4v')):
                self.send_header("Content-type", "video/mpeg4")
            elif request_path.endswith(('.wov', '.w4a')):
                self.send_header("Content-type", "video/quicktime")
            elif request_path.endswith(('.avi')):
                self.send_header("Content-type", "video/avi")
            elif request_path.endswith(('.flv')):
                self.send_header("Content-type", "video/x-flv")
            elif request_path.endswith(('.wma')):
                self.send_header("Content-type", "video/wma")
            elif request_path.endswith(('.vob')):
                self.send_header("Content-type", "video/vob")
            elif request_path.endswith(('.mpv', 'mpeg')):
                self.send_header("Content-type", "video/mpg")
            elif request_path.endswith(('.3gp')):
                self.send_header("Content-type", "video/3gpp")
            elif request_path.endswith(('.mp3')):
    ...
    def get_directory_contents_html(self, request_path):
        ...
        for file_name in file_list:
            full_file_name = request_path + "/" + file_name
...
def start_https_server(listening_port, basic_authentication_key, base_dir, certificate_file):
    CustomBaseHTTPRequestHandler.basic_authentication_key = "Basic " + \
        basic_authentication_key.decode("utf-8")
    CustomBaseHTTPRequestHandler.base_dir = base_dir
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print(
            "[-] USAGE: {} <PORT> <USERNAME:PASSWORD> <BASE_DIR> [CERTIFICATE FILE]".format(sys.argv[0]))
        sys.exit(1)
    ...
    start_https_server(
        listening_port, basic_authentication_key, base_dir, certificate_file)
